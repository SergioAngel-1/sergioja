...
export default function ProjectInfo({ project }: ProjectInfoProps) {
  ...
  const infoItems = [
    {
      ...
      value: project.status === 'completed' ? t('projects.completed') : project.status === 'in-progress' ? t('projects.inProgress') : t('projects.planned'),
      icon: (
  ...
  if (project.startDate) {
    infoItems.push({
      ...
      value: new Date(project.startDate).getFullYear().toString(),
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
