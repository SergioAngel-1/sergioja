...
export default function Pagination({ currentPage, totalPages, onPageChange }: PaginationProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 mt-8 md:mt-12">
      ...
      <div className="flex items-center gap-1 sm:gap-2">
        {getPageNumbers(isMobile).map((page, index) => {
          if (page === '...') {
            return (
              <span key={`ellipsis-${index}`} className="px-1 sm:px-3 text-text-muted text-xs sm:text-sm">
                ...
              </span>
            );
          }
          ...
          return (
            <motion.button
              ...
              className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg font-mono text-xs sm:text-sm transition-all duration-300 ${
                isActive
      ...
      {/* Info de página */}
      <div className="px-3 sm:px-4 py-1.5 sm:py-2 bg-background-surface/50 rounded-lg border border-white/30">
        <span className="text-[10px] sm:text-xs font-mono text-text-muted whitespace-nowrap">
