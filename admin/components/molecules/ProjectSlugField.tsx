...
export default function ProjectSlugField({
  ...
  const [manualDetectionDone, setManualDetectionDone] = useState(false);
  const [checkingSlug, setCheckingSlug] = useState(false);
  const [slugAvailability, setSlugAvailability] = useState<{ status: 'idle' | 'checking' | 'available' | 'conflict'; message?: string }>({ status: 'idle' });
  ...
  const checkSlugAvailability = useCallback(
    async (candidate: string) => {
      ...
      try {
        const response = await api.checkProjectSlug(candidate, existingSlug);
        if (response.success && response.data) {
          const { available, normalizedSlug, conflictProject } = response.data as any;
          if (available) {
            setSlugAvailability({ status: 'available' });
          } else {
            setSlugAvailability({
              status: 'conflict',
              message: conflictProject?.title
                ? `Ya existe en "${conflictProject.title}".`
                : 'URL ya utilizada.',
            });
          }
          if (normalizedSlug !== candidate) {
            setManualSlug(normalizedSlug);
          }
        } else {
          setSlugAvailability({
            status: 'conflict',
            message: response.error?.message || 'No se pudo validar la URL',
          });
        }
      } catch (error) {
        setSlugAvailability({
  ...
  useEffect(() => {
    ...
    if (existingSlug) {
      ...
      setHasManualSlug(false); // se recalculará cuando el título esté disponible
      setManualDetectionDone(false);
      setIsInitialized(true);
    } else {
      setIsInitialized(true);
      setManualDetectionDone(true);
      if (title) {
        const initialSlug = slugify(title);
        setPreviewSlug(initialSlug);
        setSlugValidation(validateSlugPreview(initialSlug));
      }
    }
  }, [existingSlug, title, isInitialized]);

  // Detectar slugs manuales una vez que el título esté disponible
  useEffect(() => {
    if (!isInitialized) return;
    if (manualDetectionDone) return;
    if (!originalSlug) return;
    if (!title) return;

    const autoGeneratedSlug = slugify(title);
    setHasManualSlug(originalSlug !== autoGeneratedSlug);
    setManualDetectionDone(true);
  }, [isInitialized, manualDetectionDone, originalSlug, title]);
  ...
  useEffect(() => {
    if (!isInitialized || !manualDetectionDone) return;
    if (isManualMode || hasManualSlug) return;
  ...
  }, [title, isManualMode, hasManualSlug, isInitialized, manualDetectionDone]);
  ...
  // Notificar slug derivado automáticamente al formulario
  useEffect(() => {
    if (!isManualMode && !hasManualSlug && previewSlug && onSlugUpdated) {
      onSlugUpdated(previewSlug);
    }
  }, [isManualMode, hasManualSlug, previewSlug, onSlugUpdated]);
  ...
  const handleManualSlugChange = (value: string) => {
    ...
    if (trimmed && validateSlugPreview(trimmed).valid) {
      scheduleAvailabilityCheck(trimmed);
    } else if (availabilityTimeout.current) {
      clearTimeout(availabilityTimeout.current);
    }
  };
  ...
  const handleSaveManualSlug = async () => {
    ...
    if (!existingSlug) {
      ...
      alerts.success('URL establecida', 'La URL personalizada se usará en la creación');
      return;
    }
        ...
        <div className={`p-3 rounded-lg ${
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              isLoading={isSavingSlug || checkingSlug}
            >
              {existingSlug ? 'Guardar URL' : 'Usar URL'}
            </Button>
            <Button
              type="button"
          ...
          {slugValidation.valid && slugAvailability.status === 'conflict' && slugAvailability.message && (
          ...
          {slugAvailability.status === 'available' && (
            <div className="mt-2 text-green-400 text-xs flex items-center gap-1">
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm-1-5a1 1 0 102 0V7a1 1 0 10-2 0v6zm1-9a1 1 0 110 2 1 1 0 010-2z" clipRule="evenodd" />
              </svg>
              URL disponible
            </div>
          )}
